...
def translate_text_with_google(text, target_lang='en'):
    """
    Best-effort translation helper using googletrans (Google Translate).
    Keeps the app working even if the dependency is missing.
    """
    try:
        from googletrans import Translator  # type: ignore
    except ImportError:
        app.logger.warning("googletrans is not installed; skipping translation.")
        return None, None

    try:
        translator = Translator()
        result = translator.translate(text, dest=target_lang)
        # result.src is the language code google thinks the text is in
        return result.text, (result.src or None)
...
def predict_proposed():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # Optional translation to English using Google Translate
            translation_text, translation_src = translate_text_with_google(text, target_lang='en')
